...
/**
 * @rundemo_name 监听Future
 * @desc 
 *       ListenableFuture顾名思义就是可以监听的Future，它是对java原生Future的扩展增强。我们知道Future表示一个异步计算任务
 *       ，当任务完成时可以得到计算结果。如果我们希望一旦计算完成就拿到结果展示给用户或者做另外的计算，就必须使用另一个线程不断的查询计算状态。这样做，
 *       代码复杂，而且效率低下。使用ListenableFuture
...
public class ListenableFutureSample {
	...
	public static void main(String[] args) {
		...
		// ListenableFuture是一个接口，它从jdk的Future接口继承，添加了void addListener(Runnable
		// listener, Executor executor)方法。
		/**
		...
		/**
		 * 推荐使用第二种方法，因为第二种方法可以直接得到Future的返回值，或者处理错误情况。本质上第二种方法是通过调动第一种方法实现的，
		 * 做了进一步的封装。
		 * 
		 * 另外ListenableFuture还有其他几种内置实现：
		 * 
		 * SettableFuture：不需要实现一个方法来计算返回值，而只需要返回一个固定值来做为返回值，
		 * 可以通过程序设置此Future的返回值或者异常信息 CheckedFuture：
		 * 这是一个继承自ListenableFuture接口，他提供了checkedGet()方法，此方法在Future执行发生异常时，可以抛出指定类型的异常。
		 */
	}
}
