...
/**
 * @rundemo_name 排序操作
 * @desc Ordering(排序)可以被看作是一个丰富的比较具有增强功能的链接，多个实用方法，多类型排序功能等。
 * @author root
 *
 */
public class OrderingSample {
	...
	public static void main(String[] args) {
		...
		dividingLine();
		/**
		 * reverse(): 返回与当前Ordering相反的排序: 　　 nullsFirst():
		 ...
		 * greatestOf(Iterable iterable, int
		 * k)：返回指定的第k个可迭代的最大的元素，按照这个从最大到最小的顺序。是不稳定的。 　　leastOf(Iterable<E>
		...
		print("naturalIntReduceOrdering.isOrdered(listtest)",
				naturalIntReduceOrdering.isOrdered(listtest));
		print("naturalIntReduceOrdering.isStrictlyOrdered(listtest)",
				naturalIntReduceOrdering.isStrictlyOrdered(listtest));

		print("naturalIntReduceOrdering.sortedCopy(listReduce)",
				naturalIntReduceOrdering.sortedCopy(listReduce));
		print("listReduce", listReduce);

		print("naturalIntReduceOrdering.isOrdered(naturalIntReduceOrdering.sortedCopy(listReduce))",
				naturalIntReduceOrdering.isOrdered(naturalIntReduceOrdering
						.sortedCopy(listReduce)));
		print("naturalIntReduceOrdering.isStrictlyOrdered(naturalIntReduceOrdering.sortedCopy(listReduce))",
				naturalIntReduceOrdering
						.isStrictlyOrdered(naturalIntReduceOrdering
								.sortedCopy(listReduce)));
